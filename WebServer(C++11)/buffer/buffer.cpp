...
/* 写指针 */
const char* Buffer::write_ptr_const() const {
...
std::string Buffer::retrieve_all_to_str() {
    std::string str(read_ptr(), readable_bytes());
    retrieve_all();
    return str;
}
...
void Buffer::append(const char* str, size_t len) {
    ...
    std::copy(str, str + len, write_ptr());
    write_pos += len;
}
...
void Buffer::append(const Buffer& buff) {
    append(buff.read_ptr(), buff.readable_bytes());
}
...
ssize_t Buffer::write_fd(int fd, int *save_errno) {
    ...
    ssize_t len = write(fd, read_ptr(), read_size);
    if(len < 0) {
        *save_errno = errno;
        return len;
    }
